...
from app.models.home_visit import (
    VisitasDomiciliarias,
    VisitasDomiciliariasPorProfesional,
)
...
class CareLinkCrud:
    ...
    def create_home_visit(self, user_id: int, user_data: dict) -> VisitasDomiciliarias:
        ...
        if not direccion or not telefono:
            print(
                f"⚠️ Usuario {user_id} no tiene dirección o teléfono válidos. No se creará visita domiciliaria automática."
            )
        ...
        try:
            ...
            if profesionales:
                ...
                profesional_activo = next(
                    (p for p in profesionales if p.estado == "Activo"), profesionales[0]
                )
    ...
    def save_family_member(self, id: int, kinship, family_member: FamilyMember):
        ...
        if family_member.acudiente and self._check_existing_acudiente(id):
            raise ValueError(
                "El usuario ya tiene un acudiente registrado. Solo puede tener un acudiente por usuario."
            )
    ...
    def save_medical_report(
        self, report: ReportesClinicos, attachments: List[UploadFile] = None
    ):
    ...
    def _update_family_member(
        ...
        was_previous_acudiente = (
            db_family_member.acudiente and not family_member.acudiente
        )
    ...
    def delete_user_medicines_by_record_id(self, record_id: int, medicine_id: int):
        medicine = self.__carelink_session.execute(
            select(MedicamentosPorUsuario).where(
                MedicamentosPorUsuario.id == medicine_id,
    ...
    def delete_user_care_by_record_id(self, record_id: int, care_id: int):
        care = self.__carelink_session.execute(
            select(CuidadosEnfermeriaPorUsuario).where(
                CuidadosEnfermeriaPorUsuario.id == care_id,
    ...
    def delete_user_intervention_by_record_id(
        ...
        intervention = self.__carelink_session.execute(
            select(IntervencionesPorUsuario).where(
                IntervencionesPorUsuario.id == intervention_id,
        ...
        return (
            self.__carelink_session.execute(
                select(FechasServicio).where(
                    ...
                    == service.id_servicio_contratado
                )
            )
            .scalars()
            .all()
    ...
    def calculate_partial_bill(
        ...
        for service_id, quantity in zip(service_ids, quantities):
            ...
            if rate is None:
                ...
                raise ValueError(
                    f"No se encontró tarifa configurada para el servicio {service_id} en el año {year}"
                )
    ...
    def create_contract(self, contract_data: ContratoCreateDTO) -> dict:
        ...
        return {"contrato": contract, "servicios_por_contrato": servicios_por_contrato}
    ...
    def _get_users_with_home_visits(self) -> List[User]:
        return (
            self.__carelink_session.execute(
                select(User).where(
                    User.is_deleted == False, User.visitas_domiciliarias == True
                )
            )
            .scalars()
            .all()
    ...
    def _get_users_without_home_visits(self) -> List[User]:
        return (
            self.__carelink_session.execute(
                select(User).where(
                    User.is_deleted == False, User.visitas_domiciliarias == False
                )
            )
            .scalars()
            .all()
    ...
    def _get_user_by_id(self, user_id: int) -> User:
        user = self.__carelink_session.execute(
            select(User).where(User.id_usuario == user_id, User.is_deleted == False)
        ).scalar_one_or_none()
        if not user:
    ...
    def _get_user_medical_nursing_by_id(self, id: int) -> CuidadosEnfermeriaPorUsuario:
        nursing = self.__carelink_session.execute(
            select(CuidadosEnfermeriaPorUsuario).where(
                CuidadosEnfermeriaPorUsuario.id == id
            )
        ).scalar_one_or_none()
        if not nursing:
            raise EntityNotFoundError(
                f"Cuidado de enfermería con ID {id} no encontrado"
            )
        return nursing
    ...
    def _get_user_medical_intervention_by_id(self, id: int) -> IntervencionesPorUsuario:
        intervention = self.__carelink_session.execute(
            select(IntervencionesPorUsuario).where(
                IntervencionesPorUsuario.id == id
            )
        ).scalar_one_or_none()
        if not intervention:
    ...
    def _get_medical_reports_by_user_id(self, user_id: int) -> List[ReportesClinicos]:
        ...
        return (
            self.__carelink_session.execute(
                select(ReportesClinicos).where(
                    ReportesClinicos.id_historiaclinica
                    == medical_record.id_historiaclinica
                )
            )
            .scalars()
            .all()
    ...
    def _get_clinical_evolution_by_evolution_id(
        ...
        if not evolution:
            raise EntityNotFoundError(
                f"Evolución clínica con ID {evolution_id} no encontrada"
            )
        return evolution

    def _get_activities(self) -> List[ActividadesGrupales]:
        return (
            self.__carelink_session.execute(select(ActividadesGrupales)).scalars().all()
        )
    ...
    def get_activity_with_users(self, activity_id: int) -> dict:
        ...
        return ActivityWithUsersDTO(
            ...
            profesional_nombres=(
                activity.profesional.nombres if activity.profesional else None
            ),
            profesional_apellidos=(
                activity.profesional.apellidos if activity.profesional else None
            ),
            tipo_actividad=(
                activity.tipo_actividad.tipo if activity.tipo_actividad else None
            ),
    ...
    def get_users_for_activity_date(self, activity_date: date) -> List[dict]:
        ...
        usuarios_cronograma = self.__carelink_session.execute(
            ...
            .join(
                CronogramaAsistenciaPacientes,
                User.id_usuario == CronogramaAsistenciaPacientes.id_usuario,
            )
            .join(
                CronogramaAsistencia,
                CronogramaAsistenciaPacientes.id_cronograma
                == CronogramaAsistencia.id_cronograma,
            )
        ...
        for user, cap in usuarios_cronograma:
            usuarios_dto.append(
                UserForActivityDTO(
                    ...
                    estado_asistencia=(
                        cap.estado_asistencia.value if cap.estado_asistencia else None
                    ),
                )
            )
    ...
    def assign_users_to_activity(
        self,
        activity_id: int,
        user_ids: List[int],
        estado_participacion: str = "PENDIENTE",
        observaciones: str = None,
    ) -> List[ActividadesUsuarios]:
    ...
    def update_user_activity_status(
        ...
        activity_user = self.__carelink_session.execute(
            select(ActividadesUsuarios).where(
                ActividadesUsuarios.id == activity_user_id
            )
        ).scalar_one_or_none()

        if not activity_user:
            raise ValueError(
                f"Asignación de actividad con ID {activity_user_id} no encontrada"
            )
    ...
    def update_service_rates(
        self, tarifas_data: List[dict]
    ) -> List[TarifasServicioPorAnio]:
        """
        ...
        for tarifa_data in tarifas_data:
            ...
            tarifa = (
                ...
                .filter(TarifasServicioPorAnio.id == tarifa_data["id"])
                .first()
            )
    ...
    def get_user_home_visits(self, user_id: int) -> List[VisitasDomiciliarias]:
        ...
        return (
            self.__carelink_session.query(VisitasDomiciliarias)
            .filter(VisitasDomiciliarias.id_usuario == user_id)
            .order_by(VisitasDomiciliarias.fecha_visita.desc())
            .all()
        )
    ...
    def get_home_visit_by_id(self, visita_id: int) -> VisitasDomiciliarias:
        ...
        visita = (
            self.__carelink_session.query(VisitasDomiciliarias)
            .filter(VisitasDomiciliarias.id_visitadomiciliaria == visita_id)
            .first()
        )
    ...
    def create_home_visit_manual(
        self, visita_data: dict, id_profesional_asignado: int = None
    ) -> VisitasDomiciliarias:
    ...
    def update_home_visit(
        self, visita_id: int, update_data: dict
    ) -> VisitasDomiciliarias:
        ...
        if (nueva_fecha and nueva_fecha != fecha_original) or (
            nueva_hora and nueva_hora != hora_original
        ):
    ...
    def _get_next_home_visit_invoice_number(self, year: int) -> str:
        ...
        last_invoice = (
            ...
            .order_by(Facturas.numero_factura.desc())
            .first()
        )
    ...
    def _update_expired_visits_status(self):
        ...
        pending_visits = (
            self.__carelink_session.query(VisitasDomiciliarias)
            .filter(VisitasDomiciliarias.estado_visita == "PENDIENTE")
            .all()
        )
        ...
        for visita in pending_visits:
            ...
            if visita.fecha_visita and visita.hora_visita:
                ...
                visita_datetime = datetime.combine(
    ...
    def _update_reprogrammed_visits_status(self):
        ...
        reprogrammed_visits = (
            self.__carelink_session.query(VisitasDomiciliarias)
            .filter(VisitasDomiciliarias.estado_visita == "REPROGRAMADA")
            .all()
        )
    ...
    def get_scheduled_home_visits_with_professionals(self) -> List[dict]:
        ...
        visitas = (
            self.__carelink_session.query(VisitasDomiciliarias)
            .options(joinedload(VisitasDomiciliarias.usuario))
            .order_by(VisitasDomiciliarias.fecha_visita.asc())
            .all()
        )
        ...
        for visita in result_visitas:
            visita_dict = {
                ...
                "paciente_nombre": (
                    f"{visita.usuario.nombres} {visita.usuario.apellidos}"
                    if visita.usuario
                    else "Sin paciente"
                ),
            }
            ...
            asignacion = (
                self.__carelink_session.query(VisitasDomiciliariasPorProfesional)
                .filter(
                    VisitasDomiciliariasPorProfesional.id_visitadomiciliaria
                    == visita.id_visitadomiciliaria,
            ...
            if asignacion:
                profesional = (
                    ...
                    .filter(Profesionales.id_profesional == asignacion.id_profesional)
                    .first()
                )
                if profesional:
                    visita_dict["profesional_asignado"] = (
                        f"{profesional.nombres} {profesional.apellidos} ({profesional.especialidad})"
                    )
    ...
    def get_user_flow_data(self):
        ...
        """
        from app.dto.v1.response.user_flow import (
            UserFlowResponseDTO,
            UserFlowStatsDTO,
            UserFlowItemDTO,
        )
        ...
        try:
            ...
            users_query = (
                ...
                .filter(
                    and_(User.visitas_domiciliarias == False, User.is_deleted == False)
                )
                .all()
            )
            ...
            for user in users_query:
                if user.id_usuario in user_contracts:
                    ...
                    visits_count = (
                        self.__carelink_session.query(CronogramaAsistenciaPacientes)
                        .join(CronogramaAsistencia)
                        .filter(
                            and_(
                                CronogramaAsistenciaPacientes.id_contrato
                                == contract_id,
                                CronogramaAsistenciaPacientes.id_usuario
                                == user.id_usuario,
                                extract("month", CronogramaAsistencia.fecha)
                                == current_month,
                                extract("year", CronogramaAsistencia.fecha)
                                == current_year,
                            )
                        )
                        .count()
            ...
            total_visits_attended = (
                self.__carelink_session.query(CronogramaAsistenciaPacientes)
                .join(CronogramaAsistencia)
                .filter(
                    and_(
                        CronogramaAsistenciaPacientes.estado_asistencia
            ...
            tasa_asistencia = (
                (total_visits_attended / total_visits_scheduled * 100)
                if total_visits_scheduled > 0
                else 0
            )
            ...
            return UserFlowResponseDTO(stats=stats, users=users_list)
    ...
    def get_quarterly_visits_data(self):
        ...
        """
        from app.dto.v1.response.quarterly_visits import (
            QuarterlyVisitsResponseDTO,
            MonthlyVisitData,
        )
        ...
        try:
            ...
            quarterly_visits = (
                ...
                .filter(
                    and_(
                        extract("year", VisitasDomiciliarias.fecha_visita)
                        == current_year,
                        extract("month", VisitasDomiciliarias.fecha_visita)
                        >= quarter_start_month,
                        extract("month", VisitasDomiciliarias.fecha_visita)
                        <= quarter_end_month,
            ...
            if total_quarterly_visits > 0:
                ...
                average_daily_visits = round(
                    total_quarterly_visits / days_in_quarter, 1
                )
            else:
            ...
            for i in range(6):
                ...
                monthly_data.append(
                    MonthlyVisitData(
                        month=month_names[target_month - 1], visits=month_visits
                    )
                )
            ...
            if previous_month_visits > 0:
                growth_percentage = round(
                    (
                        (current_month_visits - previous_month_visits)
                        / previous_month_visits
                    )
                    * 100,
                    1,
    ...
    def get_monthly_payments_data(self):
        ...
        """
        from app.dto.v1.response.monthly_payments import (
            MonthlyPaymentsResponseDTO,
            MonthlyPaymentData,
        )
        from datetime import datetime, date, timedelta
        from sqlalchemy import and_, extract, func
        ...
        try:
            ...
            for i in range(6):
                ...
                month_payments = (
                    self.__carelink_session.query(func.sum(Pagos.valor))
                    .filter(
                        and_(
                ...
                if i == 0:  # Mes actual
                    goal = (
                        ...
                        if previous_month_payments > month_payments
                        else month_payments
                    )
                ...
                achievement_percentage = (
                    (month_payments / goal * 100) if goal > 0 else 0
                )
            ...
            overall_goal_achievement = (
                sum(item.achievement_percentage for item in monthly_data)
                / len(monthly_data)
                if monthly_data
                else 0
            )
    ...
    def get_operational_efficiency_data(self):
        ...
        """
        from app.dto.v1.response.operational_efficiency import (
            OperationalEfficiencyResponseDTO,
        ...
        try:
            ...
            for i in range(6):
                ...
                home_visits_completion = (
                    (completed_home_visits / total_home_visits * 100)
                    if total_home_visits > 0
                    else 0
                )
                ...
                contract_management = (
                    (active_contracts / total_contracts * 100)
                ...
                billing_efficiency = (
                    (paid_bills / total_bills * 100) if total_bills > 0 else 0
                )
                ...
                efficiency = (
                    ...
                    + home_visits_completion
                    * 0.25  # 25% peso para visitas domiciliarias
            ...
            overall_efficiency = (
                sum(item.efficiency for item in monthly_data) / len(monthly_data)
                if monthly_data
                else 0
            )
            ...
            growth_percentage = (
                (
                    (current_month_efficiency - previous_month_efficiency)
                    / previous_month_efficiency
                    * 100
                )
                if previous_month_efficiency > 0
                else 0
            )

            # Calcular métricas actuales para el mes actual
            current_attendance_rate = (
                monthly_data[-1].attendance_rate if monthly_data else 0
            )
            current_home_visits_completion = (
                monthly_data[-1].home_visits_completion if monthly_data else 0
            )
            current_contract_management = (
                monthly_data[-1].contract_management if monthly_data else 0
            )
            current_billing_efficiency = (
                monthly_data[-1].billing_efficiency if monthly_data else 0
            )
    ...
    def get_user_by_document(self, document: str) -> User | None:
        ...
        return self.__carelink_session.execute(
            select(User).where(User.n_documento == document, User.is_deleted == False)
        ).scalar_one_or_none()
    ...
    def get_family_member_by_document(self, document: str) -> FamilyMember | None:
        ...
        return self.__carelink_session.execute(
            select(FamilyMember).where(
                FamilyMember.n_documento == document, FamilyMember.is_deleted == False
            )
        ).scalar_one_or_none()
    ...
    def create_family_member_relationship(
        self, user_id: int, family_member_id: int, parentesco: str
    ) -> FamiliaresYAcudientesPorUsuario:
        """Crear relación entre usuario y familiar"""
        relationship = FamiliaresYAcudientesPorUsuario(
            id_usuario=user_id, id_acudiente=family_member_id, parentesco=parentesco
        )
    ...
    def update_user_contact_info(self, user: User, family_member: FamilyMember):
        ...
        if family_member.telefono:
            ...
            telefono = (
                family_member.telefono[:20]
                if len(family_member.telefono) > 20
                else family_member.telefono
            )
        ...
        if family_member.direccion:
            ...
            direccion = (
                family_member.direccion[:150]
                if len(family_member.direccion) > 150
                else family_member.direccion
            )
            user.direccion = direccion
        self.__carelink_session.add(user)
        self.__carelink_session.flush()

    def _get_professional_by_user_id(self, user_id: int) -> Profesionales:
        professional = (
            self.__carelink_session.query(Profesionales)
        ...
        if professional is None:
            raise EntityNotFoundError("Profesional no encontrado")
        return professional

    def _get_all_authorized_users(self) -> List[AuthorizedUsers]:
        ...
        try:
            ...
            if not contrato and factura.id_visita_domiciliaria:
                visita_domiciliaria = (
                    self.__carelink_session.query(VisitasDomiciliarias)
                    .filter(
                        VisitasDomiciliarias.id_visitadomiciliaria
                        == factura.id_visita_domiciliaria
                    )
                    .first()
                )
            ...
            if usuario:
                ...
                acudientes_relacion = (
                    ...
                    .filter(
                        FamiliaresYAcudientesPorUsuario.id_usuario == usuario.id_usuario
                    )
                    .all()
                )
                ...
                for relacion in acudientes_relacion:
                    ...
                    if acudiente:
                        acudientes.append(
                            {"acudiente": acudiente, "parentesco": relacion.parentesco}
                        )
            ...
            if usuario:
                ...
                for cronograma in cronograma_paciente:
                    cronograma_info = (
                        self.__carelink_session.query(CronogramaAsistencia)
                        .filter(
                            CronogramaAsistencia.id_cronograma
                    ...
                    if cronograma_info:
                        profesional = (
                            ...
                            .filter(
                                ...
                                == cronograma_info.id_profesional
                            )
                            .first()
                        )
            ...
            detalles = (
                self.__carelink_session.query(DetalleFactura)
            ...
            for detalle in detalles:
                if detalle.id_servicio_contratado:
                    servicio_contratado = (
                        self.__carelink_session.query(ServiciosPorContrato)
                        .filter(
                            ServiciosPorContrato.id_servicio_contratado
                            == detalle.id_servicio_contratado
                        )
                        .first()
                    )
                    if servicio_contratado:
                        servicio = (
                            self.__carelink_session.query(Servicios)
                            .filter(
                                Servicios.id_servicio == servicio_contratado.id_servicio
                            )
                            .first()
                        )
            ...
            saldo_pendiente = (
                factura.total_factura - total_pagado if factura.total_factura else 0
            )
    ...
    def generate_factura_pdf(self, factura_data: dict) -> bytes:
        ...
        try:
            ...
            story.append(
                Paragraph(
                    f"<b>Número de Factura:</b> {factura.numero_factura}",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"<b>Fecha de Emisión:</b> {factura.fecha_emision}",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"<b>Fecha de Vencimiento:</b> {factura.fecha_vencimiento}",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(f"<b>Estado:</b> {factura.estado_factura}", styles["Normal"])
            )
            story.append(Spacer(1, 20))
            ...
            if factura_data["usuario"]:
                ...
                story.append(
                    Paragraph(
                        f"<b>Nombre:</b> {usuario.nombres} {usuario.apellidos}",
                        styles["Normal"],
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Documento:</b> {usuario.n_documento}", styles["Normal"]
                    )
                )
                story.append(
                    Paragraph(f"<b>Género:</b> {usuario.genero}", styles["Normal"])
                )
                story.append(
                    Paragraph(
                        f"<b>Fecha de Nacimiento:</b> {usuario.fecha_nacimiento}",
                        styles["Normal"],
                    )
                )
                if usuario.direccion:
                    story.append(
                        Paragraph(
                            f"<b>Dirección:</b> {usuario.direccion}", styles["Normal"]
                        )
                    )
                if usuario.telefono:
                    story.append(
                        Paragraph(
                            f"<b>Teléfono:</b> {usuario.telefono}", styles["Normal"]
                        )
                    )
                if usuario.email:
                    story.append(
                        Paragraph(f"<b>Email:</b> {usuario.email}", styles["Normal"])
                    )
                story.append(Spacer(1, 20))
            ...
            if factura_data["acudientes"]:
                ...
                for acudiente_info in factura_data["acudientes"]:
                    ...
                    story.append(
                        Paragraph(
                            f"<b>Nombre:</b> {acudiente.nombres} {acudiente.apellidos}",
                            styles["Normal"],
                        )
                    )
                    story.append(
                        Paragraph(
                            f"<b>Documento:</b> {acudiente.n_documento}",
                            styles["Normal"],
                        )
                    )
                    story.append(
                        Paragraph(f"<b>Parentesco:</b> {parentesco}", styles["Normal"])
                    )
                    if acudiente.telefono:
                        story.append(
                            Paragraph(
                                f"<b>Teléfono:</b> {acudiente.telefono}",
                                styles["Normal"],
                            )
                        )
            ...
            if factura_data["contrato"]:
                ...
                story.append(
                    Paragraph("<b>INFORMACIÓN DEL CONTRATO</b>", styles["Heading2"])
                )
                story.append(
                    Paragraph(
                        f"<b>ID Contrato:</b> {contrato.id_contrato}", styles["Normal"]
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Tipo:</b> {contrato.tipo_contrato}", styles["Normal"]
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Fecha de Inicio:</b> {contrato.fecha_inicio}",
                        styles["Normal"],
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Fecha de Fin:</b> {contrato.fecha_fin}", styles["Normal"]
                    )
                )
                story.append(
                    Paragraph(f"<b>Estado:</b> {contrato.estado}", styles["Normal"])
                )
                story.append(Spacer(1, 20))
                ...
                if factura_data["servicios_contratados"]:
                    story.append(
                        Paragraph("<b>SERVICIOS CONTRATADOS</b>", styles["Heading2"])
                    )

                    servicios_data = [
                        ["Servicio", "Fecha", "Descripción", "Precio por Día"]
                    ]
                    for servicio_contratado in factura_data["servicios_contratados"]:
                        ...
                        servicios_data.append(
                            [
                                ...
                                (
                                    str(servicio_contratado.fecha)
                                    if servicio_contratado.fecha
                                    else "N/A"
                                ),
                                servicio_contratado.descripcion or "Sin descripción",
                                (
                                    f"${servicio_contratado.precio_por_dia:,.2f}"
                                    if servicio_contratado.precio_por_dia
                                    else "$0.00"
                                ),
                            ]
                        )
            ...
            if factura_data["visita_domiciliaria"]:
                ...
                story.append(
                    Paragraph(
                        "<b>INFORMACIÓN DE VISITA DOMICILIARIA</b>", styles["Heading2"]
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>ID Visita:</b> {visita.id_visitadomiciliaria}",
                        styles["Normal"],
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Fecha de Visita:</b> {visita.fecha_visita}",
                        styles["Normal"],
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Hora de Visita:</b> {visita.hora_visita}", styles["Normal"]
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Estado:</b> {visita.estado_visita}", styles["Normal"]
                    )
                )
                story.append(
                    Paragraph(
                        f"<b>Dirección:</b> {visita.direccion_visita}", styles["Normal"]
                    )
                )
                if visita.telefono_visita:
                    story.append(
                        Paragraph(
                            f"<b>Teléfono:</b> {visita.telefono_visita}",
                            styles["Normal"],
                        )
                    )
                story.append(
                    Paragraph(
                        f"<b>Valor por Día:</b> ${visita.valor_dia:,.2f}",
                        styles["Normal"],
                    )
                )
                if visita.observaciones:
                    story.append(
                        Paragraph(
                            f"<b>Observaciones:</b> {visita.observaciones}",
                            styles["Normal"],
                        )
                    )
                story.append(Spacer(1, 20))
            ...
            if factura_data["cronograma_detalles"]:
                story.append(
                    Paragraph("<b>CRONOGRAMA DE ASISTENCIA</b>", styles["Heading2"])
                )
                ...
                for cronograma_info in factura_data["cronograma_detalles"]:
                    ...
                    requiere_transporte = (
                        "Sí" if cronograma_paciente.requiere_transporte else "No"
                    )
                ...
                cronograma_table = Table(
                    cronograma_data, colWidths=[2 * inch, 1.5 * inch, 3 * inch]
                )
                cronograma_table.setStyle(
                    TableStyle(
                        [
                            ("BACKGROUND", (0, 0), (-1, 0), colors.grey),
                            ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke),
            ...
            if factura_data["servicios_detalle"]:
                story.append(
                    Paragraph(
                        "<b>DETALLES DE SERVICIOS FACTURADOS</b>", styles["Heading2"]
                    )
                )
                ...
                for item in factura_data["servicios_detalle"]:
                    ...
                    descripcion = (
                        servicio.nombre
                        if servicio
                        else detalle.descripcion_servicio or "Servicio"
                    )
                ...
                # Agregar totales
                table_data.append(
                    ["", "", "<b>Subtotal:</b>", f"<b>${factura.subtotal:,.2f}</b>"]
                )
                table_data.append(
                    ["", "", "<b>Impuestos:</b>", f"<b>${factura.impuestos:,.2f}</b>"]
                )
                table_data.append(
                    ["", "", "<b>Descuentos:</b>", f"<b>${factura.descuentos:,.2f}</b>"]
                )
                table_data.append(
                    ["", "", "<b>TOTAL:</b>", f"<b>${factura.total_factura:,.2f}</b>"]
                )
            ...
            if factura_data["pagos"]:
                ...
                for pago in factura_data["pagos"]:
                    ...
                    metodo = (
                        self.__carelink_session.query(MetodoPago)
                        .filter(MetodoPago.id_metodo_pago == pago.id_metodo_pago)
                        .first()
                    )
                    tipo = (
                        self.__carelink_session.query(TipoPago)
            ...
            story.append(
                Paragraph(
                    f"<b>Total Factura:</b> ${factura.total_factura:,.2f}",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"<b>Total Pagado:</b> ${factura_data['total_pagado']:,.2f}",
                    styles["Normal"],
                )
            )
            story.append(
                Paragraph(
                    f"<b>Saldo Pendiente:</b> ${factura_data['saldo_pendiente']:,.2f}",
                    styles["Normal"],
                )
            )
            ...
            story.append(
                Paragraph(
                    f"<i>Documento generado el {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}</i>",
                    styles["Normal"],
                )
            )
